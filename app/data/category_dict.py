...
CATEGORY_DICT_BASE: dict[str, set[str]] = {
    ...
    "맛": {
        # 맛 세부 표현 — 메뉴명(음식)과 분리하여 TF-IDF 차별점 보존
        "육즙", "풍미", "식감", "바삭", "담백", "꾸덕함", "꾸덕",
        "산미", "짭짤", "달콤", "고소함", "고소", "쫄깃", "쫄깃함",
        "부드러움", "부드럽", "신선함", "신선", "진함", "깔끔함", "깔끔",
        "느끼", "향", "단맛", "짠맛", "매운맛", "맵기", "촉촉",
    },
    "장소": {
...
# ── 4. 키워드 블랙리스트 ──────────────────────────────────────────────────────
# 카테고리 분류용으로 사전에는 포함되지만, 추천 키워드로는 쓸 수 없는 범용 상위 개념어.
# 구체적 메뉴·공간·행동어 등은 제외하고, 카테고리 자체를 지칭하는 단어만 등록.
...
INDUCEMENT_TEMPLATE: dict[str, dict] = {
    ...
    "장소":   {"purpose": "search",    "inducements": ["가볼만한곳", "추천", "핫플", "명소",
                                                     "데이트", "놀거리"]},
    "맛":     {"purpose": "marketing", "inducements": []},
